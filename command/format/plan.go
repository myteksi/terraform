	...
	if diff1Resource.Action == terraform.DiffCreate {
		for _, attr := range diff1Resource.Attributes {
			...
			if attr.NewValue != p.getAttributValue(maps, diff2Resource, attr.Path) {
				changes = true
				fmt.Println(attr.Path, "\t", attr.NewValue, "\t", p.getAttributValue(maps, diff2Resource, attr.Path), "(mis-match)")
			}
		}
	} else if diff2Resource.Action == terraform.DiffCreate {
		for _, attr := range diff2Resource.Attributes {
			...
			if attr.NewValue != p.getAttributValue(maps, diff1Resource, attr.Path) {
				changes = true
				fmt.Println(attr.Path, "\t", p.getAttributValue(maps, diff1Resource, attr.Path), "\t", attr.NewValue, "(mis-match)")
			}
		}
	} else {
		for k, v := range maps[diff1Resource.Addr.Type][diff1Resource.Addr.String()] {
			if v == "" {
				continue
			}

			if v != p.getAttributValue(maps, diff2Resource, k) {
				changes = true
				fmt.Println(k, "\t", v, "\t", p.getAttributValue(maps, diff2Resource, k), "(mis-match)")
			}
		}
	}
	if changes == false {
